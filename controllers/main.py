...
class RequisicaoFachada(http.Controller):
    ...
    @http.route('/facades_request', auth='public', type="http", website=True, csrf=False)
    def index(self, **kw):


        users = http.request.env['res.users'].sudo().search([])
        partners = http.request.env['res.partner'].sudo().search([])
        facade_ad_type_ids = http.request.env['kami_sm.attendance.ad_type']
        partners = request.env['res.partner'].sudo().search([])
        
        return http.request.render('kami_forms.facades_request', {
            'partners': partners,
            'users': users.search([]),
            'partners': partners.search([]),
            'facade_ad_type_ids': facade_ad_type_ids.search([]),
        })
